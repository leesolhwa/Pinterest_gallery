window.addEventListener("load", () => {
	...
	//각 article을 클릭하면 팝업창이뜨게
	const items = document.querySelectorAll("article"); //각 article들을 변수에(배열)
	const pop = document.querySelector("#popup"); //각 article들을 변수에(배열)
	...
	for( const aa of items){
		aa.addEventListener('click',(e)=>{
			...
			if( winWidth > 767){//화면 너비가 767px보다 컸을때만 적용

				//클릭한 article img 의 src값, h2,p를 변수에 저장
